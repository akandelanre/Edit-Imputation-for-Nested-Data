...
Estimands <- c("Same race household, n_i = 2", "Same race household, n_i = 3", 
               ...
               "HH older than spouse, White HH","White HH with hisp origin",
               ...
               "At least one stepchild, black couple")
...
for(kk in 1:n){
  ...
  if(sum(hh_check[,"Relate"]==STEPCHILD)>=1 &&
     ...
     sum(hh_check[,"Relate"]==HEAD & hh_check[,"Race"]==BLACK)==1){
    Probs[30 + length(H)] <- Probs[30 + length(H)] + 1 #At least one stepchild, black couple
  }
}
...
for(k in 1:mm){
  ...
  for(kk in 1:n){
    ...
    if(sum(hh_check[,"Relate"]==STEPCHILD)>=1 &&
       sum(hh_check[,"Relate"]==SPOUSE & hh_check[,"Race"]==BLACK)==1 &&
       sum(hh_check[,"Relate"]==HEAD & hh_check[,"Race"]==BLACK)==1){
      Probs_syn[30 + length(H),k] <- Probs_syn[30 + length(H),k] + 1 #At least one stepchild, black couple
    }
  }
...
for(k in 1:mm){
  ...
  for(kk in 1:n){
    ...
    if(sum(hh_check[,"Relate"]==STEPCHILD)>=1 &&
       sum(hh_check[,"Relate"]==SPOUSE & hh_check[,"Race"]==BLACK)==1 &&
       sum(hh_check[,"Relate"]==HEAD & hh_check[,"Race"]==BLACK)==1){
      Probs_syn_weighted[30 + length(H),k] <- Probs_syn_weighted[30 + length(H),k] + 1 #At least one stepchild, black couple
    }
  }
...
if(weight_option){
  ...
  colnames(CompareProbs) <- 
    c("Orig. Data Q","Model Q","Weighted Sampler Q",
  ...
  colnames(CompareProbs) <- c("Orig. Data Q","Model Q","Orig. Data L","Orig. Data U","Model L","Model U")
}
...
Q_values <- round(CompareProbs[,1:(ncol(CompareProbs)/3)],3)
L_values <- round(CompareProbs[,seq((ncol(CompareProbs)/3)+1,ncol(CompareProbs),by=2)],3)
U_values <- round(CompareProbs[,seq((ncol(CompareProbs)/3)+2,ncol(CompareProbs),by=2)],3)
...
library(xtable)
#xtable(Q_values,digits=3)
#xtable(L_values,digits=3)
#xtable(U_values,digits=3)
